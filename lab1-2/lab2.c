...
int main() {
	...
	do {
		...
		if ((row1 <= 0) || (row2 <= 0) || (col1 <= 0) || (col2 <= 0)) {	// проверка на допустимость размеров матрицы
	...
	for (i = 0; i < row1; i++) {	// освобождение памяти под строку 
		free(A[i]);
		free(C[i]);
	}
	free(A);	free(B);	free(C);	// освобождение памяти под указатели на строки
